...
export default function ResponsiveDialog({ currUser, notify }) { 
    ...
    const complainRef = doc(db, "empowerkids_users", currUser.key);
            ...
            >
                ...
                <DialogContent>
                    <DialogContentText>
                        ...
                        <Typography variant="body2" sx={{ display: "inline" }}>{currUser.stateName}<br /></Typography>
                        <Typography variant="subtitle2" sx={{ display: "inline" }}>Age: </Typography>
                        <Typography variant="body2" sx={{ display: "inline" }}>{currUser.age}<br /></Typography>
                        ...
                        <Box sx={{ display: "flex", flexFlow: "row wrap", alignItems: "center", justifyContent: "center", gap: 3 }}>
                            ...
                            <Button variant="outlined" color="warning" onClick={async () => { await updateDoc(complainRef, { accountType: "Empowerkids Team" }); notify("Account Type Changed!", "info") }}>
                                Empowerkids Team
                            </Button>
                            <Button variant="outlined" color="warning" onClick={async () => { await updateDoc(complainRef, { accountType: "Legal Expert" }); notify("Account Type Changed!", "info") }}>
                                Legal Expert
                            </Button>
                            <Button variant="outlined" color="warning" onClick={async () => { await updateDoc(complainRef, { accountType: "Government Official" }); notify("Account Type Changed!", "info") }}>
                                Government Official
                            </Button>
