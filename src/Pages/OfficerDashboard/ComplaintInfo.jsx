...
export default function ResponsiveDialog({ complaintData, myinfo, notify }) {
    ...
    const complainRef = doc(db, "empowerkids_complaint", complaintData.key);
            ...
            >
                ...
                <DialogContent>
                    <DialogContentText>
                        ...
                        <Typography variant="body2" sx={{ display: "inline" }}>{complaintData.stateName}<br /></Typography>
                        <Typography variant="subtitle2" sx={{ display: "inline" }}>Age: </Typography>
                        <Typography variant="body2" sx={{ display: "inline" }}>{complaintData.age}<br /></Typography>
                        ...
                        <Box sx={{ display: "flex", flexFlow: "row wrap", alignItems: "center", justifyContent: "center", gap: 3 }}>
                            ...
                            {
                                myinfo.accountType === "Empowerkids Team" ?
                                    <>
                                        <Button variant="outlined" color="secondary" onClick={async () => { await updateDoc(complainRef, { issuedTo: "Legal Expert" }); notify("Complain promoted to Legal Expert!", "success") }}>
                                            Esclate
                                        </Button>
                                    </> : <></>
                            }
                            {
                                myinfo.accountType === "Legal Expert" ? 
                                    <>
                                        <Button variant="outlined" color="secondary" onClick={async () => { await updateDoc(complainRef, { issuedTo: "Government Official" }); notify("Complain promoted to Government Official!", "success") }}>
                                            Esclate
                                        </Button>
                                    </> : <></>
                            }
