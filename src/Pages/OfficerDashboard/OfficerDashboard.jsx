...
const SideNotification = ({ notify }) => {
    ...
    useEffect(() => {
        const q = query(collection(db, "empowerkids_notification"));
        const unsubscribe = onSnapshot(q, (querySnapshot) => {
            let tempNotific = []
            querySnapshot.forEach((doc) => {
    ...
    const addNotification = async () => {
        ...
        try {
            await addDoc(collection(db, "empowerkids_notification"), {
                            ...
                            return (
                                <>
                                    <ListItem>
                                        ...
                                        <Button variant="outlined" sx={{marginRight:1}} color="error" onClick={async () => { await deleteDoc(doc(db, `empowerkids_notification`, notific.key)); notify("Notification Deleted", "success") }}>
                                            <DeleteIcon />
                                        </Button>
                                        <ListItemText
